...
addSubjectInfo()
{
  ...
  this.http.post<any>('http://localhost:9033/api/subject',this.myForm.value).subscribe(
    response=>{
...
{
  console.log(this.myForm);
  this.http.put<any>(`http://localhost:9033/api/subject/${this.id}`,this.myForm.value).subscribe(
    response=>{
      Swal.fire({
