...
export class CodingComponent {
  ...
  runCode(code:string) {
    ...
    this.http
      .get(`http://54.64.6.102:9033/api/getalltestcases/${this.id}`)
      .subscribe((data) => {
        ...
        this.Testcase?.forEach((a) => {
          ...
          this.apiService
            .runCode(code, this.selectedLanguage, a.input)
            .subscribe(
              (response) => {
  ...
  submitCode(code: string) {
    ...
    if (!this.selectedLanguage) {
      Swal.fire(
        'Fill All Fields',
        'Must be select the any programming language',
        'error'
      );
      return;
    }
    ...
    const javaUrl='http://54.64.6.102:9033/api/savecode'
    this.userCode = {
      language: 'java',
      userInputCode: code,
